...
14. **Original Sand Physics.ips**: Restores the sand physics from the original Super Metroid. This makes it so that sequence breaking into Maridia from the other side isn't possible.
15. **Original Underwater Walljump.ips**: Restores the original physics for walljumping while underwater to avoid sequence breaking Maridia.
16. **No Boss Tiles on Map.ips**: Removes the Boss tiles from the Map to avoid spoiling any upcoming bosses while checking the map.
17. **No Colored Doors on Map.ips**: Remove the colour-coded doors from the Map, like in the original Super Metroid.
